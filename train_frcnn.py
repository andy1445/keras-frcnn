...
elif options.parser == 'simple' or C.use3d == True:
...
if C.use3d == False:
    ...
    model_rpn.compile(optimizer=optimizer,
                      loss=[losses.rpn_loss_cls3d(num_anchors), losses.rpn_loss_regr3d(num_anchors)])
    model_classifier.compile(optimizer=optimizer_classifier,
                             loss=[losses.class_loss_cls3d, losses.class_loss_regr3d(len(classes_count) - 1)],
                             metrics={'dense_class_{}'.format(len(classes_count)): 'accuracy'})
        ...
        try:
            if len(rpn_accuracy_rpn_monitor) == epoch_length and C.verbose:
                ...
                if mean_overlapping_bboxes == 0:
                    print(
                        'RPN not producing bounding boxes that overlap the ground truth boxes. Check RPN settings or keep training.')
            ...
            if X2 is None:
                ...
                continue
            # pick positive and negative examples to train classifier on
            ...
            loss_class = model_classifier.train_on_batch([X,
                                                          X2[:, sel_samples, :]],
                                                         [Y1[:, sel_samples, :],
                                                          Y2[:, sel_samples, :]])
