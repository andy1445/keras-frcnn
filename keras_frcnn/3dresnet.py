...
def identity_block(input_tensor, kernel_size, filters, stage, block, trainable=True):
    ...
    x = Convolution3D(nb_filter1, (1, 1, 1), name=conv_name_base + '2a', trainable=trainable)(input_tensor)
...
def nn_base(input_tensor=None, trainable=False):
    ...
    x = Convolution3D(64, (7, 7, 7), strides=2, name='conv1', trainable=trainable)(x)
    x = BatchNormalization(axis=bn_axis, name='bn_conv1')(x)
    x = Activation('relu')(x)
    x = MaxPooling3D((3, 3, 3), strides=2)(x)

    x = conv_block(x, 3, [64, 64, 64, 256], stage=2, block='a', strides=1, trainable=trainable)
    x = identity_block(x, 3, [64, 64, 64, 256], stage=2, block='b', trainable=trainable)
    x = identity_block(x, 3, [64, 64, 64, 256], stage=2, block='c', trainable=trainable)

    x = conv_block(x, 3, [128, 128, 128, 512], stage=3, block='a', trainable=trainable)
    x = identity_block(x, 3, [128, 128, 128, 512], stage=3, block='b', trainable=trainable)
    x = identity_block(x, 3, [128, 128, 128, 512], stage=3, block='c', trainable=trainable)
    x = identity_block(x, 3, [128, 128, 128, 512], stage=3, block='d', trainable=trainable)

    x = conv_block(x, 3, [256, 256, 256, 1024], stage=4, block='a', trainable=trainable)
    x = identity_block(x, 3, [256, 256, 256, 1024], stage=4, block='b', trainable=trainable)
    x = identity_block(x, 3, [256, 256, 256, 1024], stage=4, block='c', trainable=trainable)
    x = identity_block(x, 3, [256, 256, 256, 1024], stage=4, block='d', trainable=trainable)
    x = identity_block(x, 3, [256, 256, 256, 1024], stage=4, block='e', trainable=trainable)
    x = identity_block(x, 3, [256, 256, 256, 1024], stage=4, block='f', trainable=trainable)
